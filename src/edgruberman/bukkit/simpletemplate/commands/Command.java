...
class Command  {
    ...
    protected void registerAction(final Action action, final boolean isDefault) {
        if (action == null)
            throw new IllegalArgumentException("Action for " + this.command.getLabel() + " command can not be null.");
        
        if (this.actions.containsKey(action.name))
            throw new IllegalArgumentException("Action " + action.name + " of " + this.command.getLabel() + " is already registered.");
